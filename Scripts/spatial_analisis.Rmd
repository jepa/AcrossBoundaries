...
grid_eez_sw_sf <- grid_eez_sw_sf %>% 
  # st_read(my_path("D","Spatial/grid_eez_sw_sf", name = "grid_eez_sw_sf.shp")) %>%
...
grid_eez_fp_sf <- grid_eez_sf_fp %>% 
  # st_read(my_path("D","Spatial/grid_eez_fp_sf", name = "grid_eez_fp.shp")) %>% 
  select(index,state,geometry) %>% 
...
ggsave(filename = "buffer_figureb.jpg",
       plot = combined_plot,
...
# Load grid df
# For fishing ports
inter_grid <- my_path("D","Spatial","grid_eez_fp_df.csv", read = T)
...
spp_paths <- list.files(my_path("R",extra_path = "Partial/Interpolation/sw"))
...
taxon_sw <- my_path("R",extra_path = "Partial/Interpolation/sw", name = taxon_read)
...
prop_chng_map_diff <- land_sf %>% 
  ...
  ggplot() +
  geom_sf(aes(fill = mean_per)) +
  viridis::scale_fill_viridis("Change in proportion\n relative to 1980-2001 (%)", 
                              alpha = 0.8,
                              # option = "C"
                              limits = c(-100,100),
                              breaks = seq(-100,100,25),
...
season_diff <- state_change %>% 
  spread(spatial,change) %>% 
  mutate(difference_season = (fp-sw)/((fp+sw)/2)*100,
         difference_season = ifelse(difference_season > 100,100,
                                  ifelse(difference_season < -100,-100,difference_season)
         )
  ) %>% 
  select(state, category = region, diff = difference_season)

# Estimate regional differences in percentage change
spatial_diff <- state_change %>% 
  spread(region,change) %>%  
  mutate(difference_spat = (`Northeast US Fall`-`Northeast US Spring`)/((`Northeast US Fall`+`Northeast US Spring`)/2)*100,
         difference_spat = ifelse(difference_spat > 100,100,
                                 ifelse(difference_spat < -100,-100,difference_spat)
         )
  ) %>% 
  select(state,category = spatial,diff = difference_spat)
...
ggsave(filename = "proportion_chng_agg.jpg",
         plot = prop_chng_map,
         path = my_path("R","Figures"),
         width = 10,
         height = 10
