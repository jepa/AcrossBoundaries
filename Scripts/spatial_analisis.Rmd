...
# Include figure for paper

land_sf$group <- factor(state_mpa$abrev,      # Reordering group factor levels
                          levels = state_lat$abrev)


ggplot() +
  geom_sf(data = st_as_sf(map("state", plot = FALSE, fill = TRUE))) +
  geom_sf(data = land_sf, aes(fill = state)) +
...
ports_path <- my_path("G","AcrossBoundaries/Data/NOAA", name = "PalaciosJ - BSB, SFlounder, Scup by Port 1970-2020 - 2021.10.25.xlsx")
...
x <- as.data.frame(st_read(my_path("D", "Partial/grid_fp",name="grid_eez_fp.shp"))) %>% 
  select(index,spp = spcs_nm,port_name = port_nm, abrev = stt_pst) %>% 
  bind_cols(
    as.data.frame(st_coordinates(st_read(my_path("D", "Partial/grid_fp",name="grid_eez_fp.shp"))))
  ) %>% 
  rename(lon = X, lat = Y) %>% 
  left_join(as.data.frame(land_sf),
            by = "abrev") %>% 
...
grids_sw <- bind_rows(
  my_path("G", "AcrossBoundaries/Data/Partial/grid_sw", name = "grid_eez_sw_df.csv", read = T) %>% 
...
grids_fp <- my_path("G", "AcrossBoundaries/Data/Partial/grid_fp", name = "grid_eez_fp_df.csv", read = T
) %>%
  mutate(spatial = "fp",
         spp = ifelse(spp == "summer flounder","Paralichthys dentatus", 
                      ifelse(spp == "scup", "Stenotomus chrysops", 
...
spp_paths <- list.files(my_path("G",extra_path = "AcrossBoundaries/Results/Partial/Interpolation/"))
...
taxon_sw <- my_path("G",extra_path = "AcrossBoundaries/Results/Partial/Interpolation/", name = taxon_read)
...
land_sf_t$group <- factor(land_sf_t$abrev,      # Reordering group factor levels
                               levels = state_lat$abrev)
...
study_plot <-
  ...
  geom_sf(data = land_sf_t, aes(fill = reorder(name,-order))) +
...
my_path("D","Partial","top_ports.csv", read = T) %>% 
  filter(tresh == "25th",
         state_postal == "NY") %>% 
  ggplot() +
  geom_point(
...
grid_eez_sw_sf <-  st_read(my_path("G","AcrossBoundaries/Data/Partial/grid_sw", name = "grid_eez_sw_sf.shp")) %>%
...
grid_eez_fp_sf <-  st_read(my_path("G","AcrossBoundaries/Data/Partial/grid_fp", name = "grid_eez_fp.shp")) %>%
...
top_ports_sen <- my_path("G","AcrossBoundaries/Data/Partial","top_ports.csv", read = T)
...
sens_plot <- grid_fp_bf_dt %>% 
  ...
  labs(y = "Number of grid-cells (n)", x = "Threshold") +
