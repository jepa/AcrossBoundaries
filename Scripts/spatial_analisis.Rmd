...
wpi_sf <- st_as_sf(wpi_data,
             coords = c("longitude", "latitude"),
             crs = 4326)

# Merge data with state waters to,locate ports 
port_sf <- st_join(wpi_sf,state_sf) %>% 
...
We set a buffer of *4* deg (410 km, ~ 221 nm) that overshoots the EEZ a bit, but is eventually cropped

```{r buffer, eval = T, echo = T}

# Buffer state waters
ports_bf = st_buffer(port_sf, 4) %>% 
  st_transform(4326) %>% # to match shape
  st_set_crs(4326)
...
ggplot() +
  geom_sf(data = eez_sf, aes(),fill = NA) +
  geom_sf(data = ports_bf, aes(color = state), fill = NA) +
  scale_color_manual(values = state_pallet)+
  theme_dark()
...
Once we have a gridded area, we converted the grid to a `sf` so we can merge it with the buffered statesand ports and finally filter out everything outside the states polygon
