...
library(MyFunctions)
MyFunctions::my_lib(c("ggmap","sf","st","tidyverse","tools","readr","data.table","maps","viridis","wesanderson","knitr","kableExtra","plotly","ggrepel","ggsflabel","janitor","interp","rnaturalearth"))
```
...
path_world <- "~/OneDrive - UW-Madison/Spatial_data/World_EEZ_v11_20191118/"

# The File
fnam_world <- "eez_boundaries_v11.shp"
...
eez_sf <- st_read(dsn = path_world,
  ...
  filter(TERRITORY1 == "United States",
         LENGTH_KM == 2734) #%>%
  # filter(str_detect(EEZ1, "United States")) %>%
...
eez_sf <- st_read("/Volumes/Enterprise/Data/Spatial/World_EEZ_v11_20191118/eez_v11.shp") %>%
  # rename(eez_name = Name) %>%
  # st_transform(crs = 4326) %>% # 4326
  filter(TERRITORY1 == "United States") %>% 
  st_crop(xmin = -85, xmax = -60, ymin = 30, ymax = 50)
...
So, in cases qhere *ei* = 1, the quota is aligned with the distribution; when *ei* > 1 then the historic quota is more than the stock's State's distribution, contrarily, *ei* < 1 means that the historic quota is less than what the State currently has.


**Note:** There are some crazzy outliers that have been removed, for now... 
