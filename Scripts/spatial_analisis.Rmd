...
######4.2 Data from NOAA fisheries 
https://www.fisheries.noaa.gov/foss/f?p=215:11:15514950376034::NO:::

Delaware data was colected from NOAA's [Snapshots of Human Communities and Fisheries in the Northeast
...
noaa_ports <- data.table(
  port =str_to_sentence(c(
    ...
    "PT. PLEASANT, NJ", #40.08351847675102,-73.99190094110679,
    ...
    "OCEAN CITY, MD", #38.3802922004859, -75.05089736920318
    "Indian river, DE",
  ...
  separate(port, into = c("port","abrev"), sep = ',') %>% 
  mutate(abrev = str_squish(str_to_upper(abrev))) %>% 
  left_join(as.data.frame(land_sf)) %>% 
  left_join(noaa_ports) %>% 
  select(port,state = ID,lat,lon) %>% 
...
ggplot() +
  ...
  geom_point(data = noaa_ports, aes(x = lon, y = lat)) +
  scale_color_manual(values = state_pallet) +
  scale_fill_manual(values = state_pallet) +
  theme_dark() +
...
ggsave(filename = "buffer_figurec.jpg",
       plot = combined_plot,
