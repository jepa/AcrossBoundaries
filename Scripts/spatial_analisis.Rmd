...
write_csv(grid_eez_fp_df, paste0(my_path("R", "Partial"),"grid_eez_fp_df.csv"))
...
state_order <- my_path("D","Spatial/grid_eez_fp_sf", name = "grid_eez_fp_df.csv",read = T) %>%
  group_by(state) %>% 
  summarise(
    order = mean(lat)
  ) %>% 
...
# Load grid df
# For fishing ports
grid_eez_fp_df <- my_path("D","Spatial/grid_eez_fp_sf","grid_eez_fp_df.csv", read = T)

# For state waters
# grid_eez_sw_df <- my_path("D","Spatial/grid_eez_fp_sf","grid_eez_fp_df.csv", read = T)
...
taxon_fp <- my_path("R",extra_path = "Partial/Interpolation/fp", name = taxon_read)
...
historic_spp <- bind_rows(lapply(taxon_sw, fread)) %>% 
  mutate(spatial = "sw") %>% 
  bind_rows(
    bind_rows(lapply(taxon_fp, fread))
  ) %>% 
  mutate(state = str_to_sentence(state),
...
save_plot(
  path = my_path("R","Figures"),
  "proportion_chg_spp_spatial.jpg",
  p,
  base_height = 14,
  base_width = 14
)
...
# Spatial map
agg_sppatial_map <- land_sf %>% 
  left_join(state_fit,
            by = "state") %>% 
  filter(spatial != "Difference") %>% 
  ggplot() +
  geom_sf(aes(fill = mean_per)) +
  viridis::scale_fill_viridis("Distribution\nProportion", alpha = 0.8) +
  facet_grid(spatial~ label) +
  labs(x = "", 
       y = "") +
...
# Difference map
agg_diff_map <- land_sf %>% 
  left_join(state_fit,
            by = "state") %>% 
  filter(spatial == "Difference") %>% 
  ggplot() +
  geom_sf(aes(fill = mean_per)) +
  viridis::scale_fill_viridis("Percentage\nDifference",option = "C", alpha = 0.8) +
  facet_grid(spatial~ label) +
  labs(x = "", 
       y = "") +
  my_ggtheme_p(facet_tx_s = 16,
...
p <- ggdraw() +
  # Revenue circular
  draw_plot(agg_sppatial_map, x = 0, y = 0.35, width = 1, height = 0.65) +
  # Catch circular
  draw_plot(agg_diff_map, x = 0.093, y = 0.05, width = 0.814, height = 0.4) +
  draw_plot_label(label = c("Latitude", "Longitude"), 
                  size = 10,
                  angle = c(90,0),
                  x = c(0,0.35), 
