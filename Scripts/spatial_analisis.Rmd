...
![Buffer figure for paper](/Volumes/Enterprise/Data/AcrossBoundaries/Results/Figures/buffer_figure_b.jpg)
...
interpol_result <- historic_spp %>% 
  ...
  group_by(label,index,spp,lat,lon) %>% 
...
inter_map <- ggplot() +
  ...
  scale_color_viridis("Mean Interpolation", alpha = 0.8, 
                      limits = c(0,40)) +
  facet_grid(label ~ spp) +
  my_ggtheme_m(map_type = "na", leg_pos = "bottom",ax_tx_s = 10,ax_tl_s = 12,leg_tx_s = 12) +
  theme(legend.key.width = unit(3,"line"))


ggsave("interpolation_fig.jpg",
       inter_map,
       path = my_path("R","Figures"),
       width = 8,
       height = 6)
...
diff_plot <- land_sf %>% 
  ...
  ggplot() +
  geom_sf(aes(fill = mean_per)) +
  viridis::scale_fill_viridis("Percentage difference", alpha = 0.8,option = "C",
                              breaks = seq(-20,70,10),
                              limits=(c(-20,70))
                              ) +
  facet_grid(spatial~ spp+label) +
...
save_plot(
  path = my_path("R","Figures"),
  "proportion_chg_spp_spatialN.jpg",
  p,
  base_height = 14,
  base_width = 14
)
