...
test <- state_sf %>% 
  filter(jurisdicti == "maine") %>% 
  select(jurisdicti,Shape)
  
ggplot(test) +
  geom_sf()

pbuf = st_buffer(test, 100)

p <- ggplot() +
  geom_sf(data = test, aes(), color = "blue") +
  geom_sf(data = pbuf, aes(), color = "red")

ggsave("testplot.png",p)


### Test with library(ggforce)
